...
public class CloudDbQueyCalls {
    ...
    private void processUserInfo(CloudDBZoneSnapshot<UsersInfoTable> snapshot, CloudDbAction cloudDbAction) {
    ...
    public void queryAllMainCategories(CloudDbAction cloudDbAction) {
        ...
        coMainCategoryQueryTask
                .addOnSuccessListener(
                        new OnSuccessListener<CloudDBZoneSnapshot<CoursesMainCategoryTable>>() {
                            @Override
                            public void onSuccess(CloudDBZoneSnapshot<CoursesMainCategoryTable> coMainCategoryCloudDBZoneSnapshot) {
                                CloudDbQueyCalls.this.processMainCategories(coMainCategoryCloudDBZoneSnapshot, cloudDbAction);
                            }
                        })
                .addOnFailureListener(
                        e -> cloudDbUiCallbackListener.onFailureDbQueryMessage
    ...
    private void processMainCategories(CloudDBZoneSnapshot<CoursesMainCategoryTable> snapshot
        ...
        CloudDBZoneObjectList<CoursesMainCategoryTable> courseMainCategoryCursor = snapshot.getSnapshotObjects();
     ...
     * @param cloudDbAction the cloud db action
     */
    public void queryCourseDetailsTable(CloudDBZoneQuery<CourseDetailsTable> query, CloudDbAction cloudDbAction) {
        if (mCloudDBZone == null) {
            return;
        }
        ...
        coDetailsQueryTask =mCloudDBZone.executeQuery(
                                                        ...
                                                        CloudDBZoneQuery.CloudDBZoneQueryPolicy.POLICY_QUERY_FROM_CLOUD_ONLY);
        /*int s1 = CloudDBZoneQuery.CloudDBZoneQueryPolicy.POLICY_QUERY_FROM_CLOUD_ONLY.getIndex();
        int s2 = CloudDBZoneQuery.CloudDBZoneQueryPolicy.POLICY_QUERY_DEFAULT.getIndex();
        System.out.println("###Sundy#"+s1+", " + s2);*/
        ...
        coDetailsQueryTask.addOnSuccessListener(coDetailsCloudDBZoneSnapshot -> {
            processCourseDetailsTable(coDetailsCloudDBZoneSnapshot, cloudDbAction);})
            .addOnFailureListener(e -> cloudDbUiCallbackListener.onFailureDbQueryMessage(cloudDbAction, ""));
    }

    private void processCourseDetailsTable(CloudDBZoneSnapshot<CourseDetailsTable> snapshot, CloudDbAction cloudDbAction) {
        CloudDBZoneObjectList<CourseDetailsTable> coDetailsCursor = snapshot.getSnapshotObjects();
    ...
    /**
     * Query com.sundydemo.learning1.java.database.tables.ExamTable
    ...
    /**
     * Query com.sundydemo.learning1.java.database.tables.
    ...
    public void queryMyCoursesTable(
        ...
        if (query == null) {
            myCoursesQueryTask =mCloudDBZone.executeQuery(
        ...
        myCoursesQueryTask.addOnSuccessListener(myCoursesCloudDBZoneSnapshot -> processMyCoursesTable(myCoursesCloudDBZoneSnapshot, cloudDbAction))
                          .addOnFailureListener(exp -> cloudDbUiCallbackListener.onFailureDbQueryMessage(cloudDbAction, ""));
    }

    private void processMyCoursesTable(CloudDBZoneSnapshot<MyCoursesTable> snapshot, CloudDbAction cloudDbAction) {
        CloudDBZoneObjectList<MyCoursesTable> myCoursesCursor = snapshot.getSnapshotObjects();
