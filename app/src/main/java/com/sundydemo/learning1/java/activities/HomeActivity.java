...
import com.sundydemo.learning1.databinding.ActivityHomeBinding;
import com.sundydemo.learning1.java.fragments.HomeFragment;
import com.sundydemo.learning1.java.fragments.MyCoursesFragment;
import com.sundydemo.learning1.java.fragments.ProfileFragment;
    ...
    public void onSuccessDbData(CloudDbAction cloudDbAction, List<CloudDBZoneObject> dataList) {
        switch (cloudDbAction) {
            case GET_MAIN_CATEGORIES:
                ...
                cloudDbHelper.getCloudDbQueyCalls().queryCourseDetailsTable(null, CloudDbAction.GET_ALL_COURSES);
                break;
    ...
    private void loadAllCourses(List<CloudDBZoneObject> dataList) {
        ...
        if (!queryRecentlyViewed()) {
            if ((getSupportFragmentManager().getFragments().get(0) instanceof HomeFragment)) {
                if (Build.VERSION.SDK_INT >= 24) {
                    ((HomeFragment) (getSupportFragmentManager().getFragments().get(0))).
                            setData(mainCategoryList,coDetailsArrayList, recentlyViewedCoursesArrayList);
                }
            }
        }
    }
