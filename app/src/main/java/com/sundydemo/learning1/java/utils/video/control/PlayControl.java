 ...
 */
package com.sundydemo.learning1.java.utils.video.control;
...
public class PlayControl {
    ...
    private void initPlayer() {
        if (LearningApplication.getWisePlayerFactory() == null) {
            return;
        }
        wisePlayer = LearningApplication.getWisePlayerFactory().createWisePlayer();
    }
     ...
     */
    public void ready() {
        Log.d("mymsg","PlayControl.ready() wisePlayer: "+ wisePlayer.toString());
        if (wisePlayer != null) {
            setHttpVideo(true);
            if (videoUrl != null) wisePlayer.setPlayUrl(new String[]{videoUrl});
    ...
    private void getBitrates(List<Integer> bitrateIntList) {
    ...
    public void setCycleMode(boolean isCycleMode) {
        if (wisePlayer != null) {
            wisePlayer.setCycleMode(
                    isCycleMode ? PlayerConstants.CycleMode.MODE_CYCLE : PlayerConstants.CycleMode.MODE_NORMAL);
        }
    }
